...
#### CALCULATE OBSERVED ABSOLUTE GROWTH RATE -----------------------------------------------------

start.tl.mean <- larval.lah %>% group_by(population, treatment, rearing.tank) %>% 
  summarize(start.mean.tl = mean(length.mm))

final.tl.mean <- larval.growth %>% group_by(population, treatment, rearing.tank) %>% 
  summarize(final.mean.tl = mean(length.mm))
  
growth.obs <- left_join(start.tl.mean, final.tl.mean) %>% 
  left_join(embryo.hatch.dates) %>% 
  mutate(tl.diff = final.mean.tl - start.mean.tl,
...
group.pairwise <- data.frame(do.call(rbind, combn(as.character(growth.boot.95perc$group), 2, simplify = FALSE))) %>% 
  rename(group1 = X1, group2 = X2)

group.pairwise.diff <- group.pairwise %>% left_join(growth.boot.95perc, by = c("group1" = "group")) %>%
  left_join(growth.boot.95perc, by = c("group2" = "group")) %>% 
...
ggplot(growth.boot.95perc.cld, aes(x = population, y = mean.growth, group = treatment, fill = treatment)) +
